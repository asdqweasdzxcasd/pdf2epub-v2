...
1. 각 페이지를 이미지로 렌더링하고, 텍스트 레이어가 있으면 그대로 읽습니다
   (API 호출·비용 없음).
2. 이미지/스캔 페이지는 스캔 페이지를 PDF 청크로 Mistral OCR에 보내 텍스트와
...
- 텍스트 PDF는 기기 밖으로 전혀 나가지 않습니다 — 무료 경로는 네트워크
  호출을 하지 않습니다.
- 이미지/스캔 PDF는 스캔 페이지 자체가 40페이지 단위 PDF 청크로 Mistral OCR API에 전송됩니다
...
- 정식 PyPI 패키지 배포 (`pip install pdf2epub-v2`)
